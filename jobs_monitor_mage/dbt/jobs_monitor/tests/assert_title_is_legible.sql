...
*/
select
    {{ clean_alphanumeric('title') }} as title -- use the clean_alphanumeric macro to remove special characters from the title
from {{ ref('int_jobs_cleaned') }} -- reference to the intermediate model that contains cleaned and categorized job listings
where title ~ '[^a-zA-Z0-9 áéíóúÁÉÍÓÚñÑ]' -- filter for titles that contain characters other than letters, numbers, spaces, and common Spanish accented characters
